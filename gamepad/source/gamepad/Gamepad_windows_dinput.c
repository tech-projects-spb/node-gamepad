...
static void disposeDevice(struct Gamepad_device *deviceRecord) {
    ...
    free((void *)deviceRecord->deviceID);
...
char* getGUUID(GUID guuid) {
  char *str = malloc(sizeof(char) * (32 + 4 + 1));

  snprintf(str, (32+4 + 1), "%08lx-%04x-%04x-", guuid.Data1, guuid.Data2, guuid.Data3);
  for (int i = 0; i < 2; i++) {
    snprintf(&str[19+i*2], 3, "%02x", guuid.Data4[i]);
  }
  snprintf(&str[23], 2, "-");
  for (int i = 0; i < 6; i++) {
    snprintf(&str[24+i*2], 7, "%02x", guuid.Data4[i+2]);
  }
...
static BOOL CALLBACK enumDevicesCallback(const DIDEVICEINSTANCE *instance, LPVOID context) {
    ...
    deviceRecord->deviceID = getGUUID(instance->guidInstance);
    ...
    if (xInputAvailable) {
        for (playerIndex = 0; playerIndex < 4; playerIndex++) {
            ...
            if (xResult == ERROR_SUCCESS && registeredXInputDevices[playerIndex] == NULL) {
                ...
                deviceRecord->privateData = deviceRecordPrivate;
                deviceRecord->deviceID = malloc(sizeof(char) * 2);
                snprintf(deviceRecord->deviceID, 2, "%i", playerIndex);
